...
/**
 * @author : choi-ys
 * @date : 2021/06/18 11:08 오전
 * @apiNote : Kotlin 스코프 함수 : run
 *  - 인스턴스를 생성 후 변수에 담기전 초기화 과정에서
 *    run 함수의 scope 내에서 참조 연산자 없이 직접 인스턴스의 속성과 함수에 접근 가능
...
fun main(){
    ...
    /**
     * run 함수의 scope에서 인스턴스의 속성과 함수 직접 접근 및 마지막 구문의 처리 수행 결과를 반환
     * 단, run scope 바깥에 해당 인스턴스와 동일한 이름의 변수나 함수가 있는 경우 scope 바깥에 우선순위를 가짐
     */
    var eventProductPrice: Int = Book(name, price).run {
